...
// ====== VOICE ======
// 使用 Android 原生 TTS 引擎（系统已安装的中文语音包）
// 通过 Capacitor nativeBridge 直接调用，不走插件 web fallback
var ttsReady = false;
var audioCtxSupported = 'AudioContext' in window || 'webkitAudioContext' in window;

// 中文数字到发音频率映射
var NUM_FREQ = {
  '零': 200, '一': 300, '二': 400, '三': 500, '四': 600,
  '五': 700, '六': 800, '七': 900, '八': 1000, '九': 1100, '十': 1200
};

function initTTS() {
  try {
    var platform = window.Capacitor ? window.Capacitor.getPlatform() : 'browser';
    console.log('TTS init: platform=' + platform);

    if (platform === 'android' && window.Capacitor) {
      // 检查 TextToSpeech 插件是否可通过 native bridge 调用
      // Capacitor 8+ 通过 Plugins.TextToSpeech 暴露
      if (window.Capacitor.Plugins && window.Capacitor.Plugins.TextToSpeech) {
        var p = window.Capacitor.Plugins.TextToSpeech;
        // 测试是否桥接到了原生（而不是 web fallback）
        if (p.speak && typeof p.speak === 'function') {
          // 进一步验证：检查内部方法名是否包含 native bridge 特征
          var methodStr = p.speak.toString();
          if (methodStr.indexOf('capacitor code') >= 0 || methodStr.indexOf('nativePromise') >= 0 || methodStr.indexOf('nativeCallback') >= 0) {
            ttsReady = true;
            console.log('TTS: native bridge confirmed');
            return;
          }
          // 即使不是 native bridge，也尝试调用（可能内部实现不同）
          ttsReady = true;
          console.log('TTS: attempting native call');
          return;
        }
      }
      // 尝试通过 registerPlugin 获取（不传 web fallback）
      try {
        var TTSPlugin = window.Capacitor.registerPlugin('TextToSpeech', {});
        if (TTSPlugin && TTSPlugin.speak) {
          ttsReady = true;
          console.log('TTS: registerPlugin succeeded, no web fallback');
          window._TTSPlugin = TTSPlugin;
        }
      } catch(e) {
        console.log('TTS registerPlugin error:', e);
      }
    }
  } catch(e) {
...
function speak(text, cb) {
  ...
  if(!text) return cb ? cb() : void 0;

  var platform = window.Capacitor ? window.Capacitor.getPlatform() : 'browser';

  // 1. 优先用 Capacitor native TTS（Android 系统 TTS 引擎）
  if (platform === 'android' && ttsReady) {
    try {
      // 通过 Plugins 对象调用
      if (window.Capacitor.Plugins && window.Capacitor.Plugins.TextToSpeech) {
        window.Capacitor.Plugins.TextToSpeech.speak({
          ...
          console.log('TTS native speak failed:', err);
          fallbackSpeak(text, cb);
        });
        return;
      }
      // 通过 registerPlugin 获取的 proxy 调用
      if (window._TTSPlugin && window._TTSPlugin.speak) {
        window._TTSPlugin.speak({
          text: text,
          lang: 'zh-CN',
          rate: 1.0,
          volume: 1.0
          ...
          console.log('TTS registerPlugin speak failed:', err);
          fallbackSpeak(text, cb);
        });
        return;
      }
  ...
  // 2. 降级：浏览器 speechSynthesis（桌面浏览器）
  ...
  // 3. 最终降级：AudioContext beep
  fallbackSpeak(text, cb);
}

function fallbackSpeak(text, cb) {
  if(!audioCtxSupported) return cb ? cb() : void 0;

  // 判断是否是纯数字（倒计时播报）
  var isNumber = /^[\d一二三四五六七八九十]+$/.test(text.trim());
  if (isNumber) {
    var nums = text.trim().split('');
    var delay = 0;
    nums.forEach(function(ch) {
      var freq = NUM_FREQ[ch] || 880;
      setTimeout(function() { playBeep(freq, 120); }, delay);
      delay += 150;
    });
    setTimeout(cb, delay);
    return;
...
function clearAllVoice() {
  try {
    if (ttsReady) {
      if (window.Capacitor.Plugins && window.Capacitor.Plugins.TextToSpeech && window.Capacitor.Plugins.TextToSpeech.stop) {
        window.Capacitor.Plugins.TextToSpeech.stop();
      }
      if (window._TTSPlugin && window._TTSPlugin.stop) {
        window._TTSPlugin.stop();
      }
    }
...
function renderPlan() {
  ...
  if (!kneeOn && !wristOn) { modeLabel = "自由组合"; modeColor = "#8892a4"; }
  else if (kneeOn && !wristOn) { modeLabel = "护膝"; modeColor = "#6bcb77"; }
  else if (!kneeOn && wristOn) { modeLabel = "护腕"; modeColor = "#4d96ff"; }
  else { modeLabel = "双保护"; modeColor = "#ff6b6b"; }
  ...
  html += '<div class="today-meta">' + totalSets + '组 · 约30分钟 · <span style="color:' + modeColor + '">' + modeLabel + '</span></div>';
  ...
  DATA.PLANS.forEach(function(p) {
    ...
    var name = p.name;
    if(acts) {
      var kOn = ST.settings.knee !== false;
      var wOn = ST.wristOn !== false;
      var mode, mColor;
      if (!kOn && !wOn) { mode = "自由组合"; mColor = "#8892a4"; }
      else if (kOn && !wOn) { mode = "护膝"; mColor = "#6bcb77"; }
      else if (!kOn && wOn) { mode = "护腕"; mColor = "#4d96ff"; }
      else { mode = "双保护"; mColor = "#ff6b6b"; }
      name += ' <span style="font-size:11px;color:' + mColor + '">(' + mode + ')</span>';
    }
...
function renderSettings() {
  ...
  DB.getSettings().then(function(saved) {
    ...
    // 同步 ST 状态（与 checkbox 保持一致）
    if (saved.knee !== undefined) ST.settings.knee = saved.knee;
    ...
    // 膝盖/手腕保护开关：实时同步 ST 并刷新计划页
    var kneeEl = document.getElementById("s-knee");
    var wristEl = document.getElementById("s-wrist");
    if (kneeEl && !kneeEl._bound) {
      kneeEl._bound = true;
      kneeEl.addEventListener("change", function() {
        ST.settings.knee = kneeEl.checked;
        syncProtectionMode();
      });
    }
    if (wristEl && !wristEl._bound) {
      wristEl._bound = true;
      wristEl.addEventListener("change", function() {
        ST.wristOn = wristEl.checked;
        syncProtectionMode();
      });
    }
  });
...
// 实时同步膝盖/手腕保护开关到 ST 并刷新计划页
function syncProtectionMode() {
  ST.settings.knee = ST.settings.knee !== false;
  ST.wristOn = ST.wristOn !== false;
  if (ST.currentView === "plan") renderPlan();
}
...
function init() {
  ...
  }).catch(function(){ console.log('init: settings load error'); });
}
